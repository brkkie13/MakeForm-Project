...
// types
import { ResponsesState, FormDetail, SubmittedResponse } from '@/types/types';

interface Props {
  formDetail?: FormDetail;
  onEdit?: () => void;
  onRemove?: () => void;
  sharedForm?: FormDetail;
  responseDetail?: FormDetail;
}
...
function Detail({
  ...
  useEffect(() => {
    if (formDetail) {
      setForm(formDetail);
      form && dispatch(responsesActions.setInitialValue(form.items));
    } else if (sharedForm) {
      setForm(sharedForm);
  ...
  const changeInputValueHandler = (itemIdx: number, event: React.FormEvent) => {
    const newValue = (event.target as HTMLInputElement).value;
    dispatch(responsesActions.changeInputValue({ itemIdx, newValue }));
  };

  const changeRatingHandler = (itemIdx: number, score: number) => {
      ...
      <div>
        ...
        {form?.responses?.map((item, itemIdx) => (
          <FormItemStyled key={itemIdx}>
            {'title' in item && <h2 className="title">{item.title}</h2>}
