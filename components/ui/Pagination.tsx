...
function Pagination({ dataListLength, currentPage, onPageChange }: Props) {
  const totalPages = Math.ceil(dataListLength / postsPerPage);
  ...
  const clickArrowHandler = (direction: string) => {
  ...
  const clickDoubleArrowHandler = (direction: string) => {
    if (direction === PREV && currentPageGroup > 1) {
