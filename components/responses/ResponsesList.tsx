...
import { Response, ResponseItemWithTitle } from '@/types/types';
...
// code
function ResponsesList({ allPosts, filteredPosts, currentPosts, user }: Props) {
  const router = useRouter();

  const showDetailPageHandler = (dataId: string) => {
  ...
  const maxLength = currentPosts.reduce((max, data) => {
    const validResponsesLength = data.responses.filter(
      ...
      (item): item is ResponseItemWithTitle =>
        'title' in item && 'response' in item
  ...
  const data = currentPosts.map(item => {
    ...
    item.responses
      .filter(
        // el => el.title && el.response
        (el): el is ResponseItemWithTitle => 'title' in el && 'response' in el
      )
